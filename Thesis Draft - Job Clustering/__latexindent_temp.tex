...
For skill sets, a different pre-process was applied to three columns $job\_description$, $job\_requirement$, and $job\_qualification$: lower casing, URL removal, replacing all special characters (including punctuations, numbers, and line break characters) with character "|", extracting top 10 key phrases (using Rake algorithm), removal of English stopwords, removal of character "|", word tokenizing, and lemmatizing. Afterward, the dataset was reshaped to long format such that one row only consists one skill per column. The skill subset contains only $job\_id$, $skill\_id$ (row number per $job\_id$) and pre-processed columns $job\_description$, $job\_requirement$, and $job\_qualification$.

\subsection{Feature selection and feature extraction}
